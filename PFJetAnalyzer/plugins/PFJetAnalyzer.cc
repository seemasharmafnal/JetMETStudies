...
#include "PFCalibration/PFJetAnalyzer/plugins/PFJetAnalyzer.h"
...
PFJetAnalyzer::PFJetAnalyzer(const edm::ParameterSet& iConfig) {
  ...
  tokenPFJets_    = consumes<std::vector<pat::Jet> >(inputTagPFJets_);
...
void PFJetAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  //for(pfJetItr ijet = pfJets->begin(); ijet!=pfJets->end(); ijet++){
...
void PFJetAnalyzer::analysePFCandidates(edm::Handle<reco::PFCandidateCollection> pfCandidates){
...
void
PFJetAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
